...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if(!has_url_param($params, 'id')){
        $sql = "SELECT `question`.`id`, `question`.`name`, `question`.`visibility` FROM `question` INNER JOIN `set` ON `question`.`setId` = `set`.`id` WHERE setId = '" . $setId . "' AND categoryId = '" . $categoryId ."'";
    } else {
        ...
        $sql = "SELECT `question`.`id`, `question`.`name`, `question`.`visibility` FROM `question` INNER JOIN `set` ON `question`.`setId` = `set`.`id` WHERE `question`.`id` = '" . $id . "' AND categoryId = '" . $categoryId . "' AND setId ='" . $setId . "' LIMIT 1";
    }
